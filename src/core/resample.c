...
asr_resampler_t* resampler_create(switch_memory_pool_t *pool, uint32_t from_rate, uint32_t to_rate) {
    ...
    if (switch_resample_create(&rs->resampler, from_rate, to_rate, 320, SWITCH_RESAMPLE_QUALITY, pool) 
...
switch_status_t resampler_process(asr_resampler_t *rs, int16_t *input, size_t input_samples, 
    ...
    switch_size_t in_len = input_samples;
    switch_size_t out_len = *output_samples;
    
    if (!rs || !rs->resampler || !input || !output || !output_samples) {
        return SWITCH_STATUS_FALSE;
    }
    
    switch_resample_process(rs->resampler, input, in_len, output, out_len);
    
    *output_samples = (size_t)rs->resampler->to_len;
